...
const bidInputs = document.querySelectorAll('input[type="number"]');
...
function placeBid(instrumentIndex) {
  ...
  if (instrumentIndex === 0) {
    ...
    const bidInput = bidInputs[instrumentIndex - 1];
  ...
  if (!isNaN(bidAmount) && bidAmount > 0) {
    ...
    if (!highestBidElement.dataset.highestBid || bidAmount > parseInt(highestBidElement.dataset.highestBid)) {
      ...
      highestBidElement.textContent = `Puja más alta: ${instrumentName} ${bidAmount} euros`;
    }
    ...
    if (instrumentIndex === 0) {
      ...
      bidInputs[instrumentIndex - 1].value = '';
    }
...
function loadBidsFromLocalStorage() {
  ...
  if (savedBids) {
    ...
    highestBidElement.textContent = `Puja más alta: ${highestBidInstrumentName} ${highestBid} euros`;
  }
}
